...
void BPlusTree::searchOne(string dbName, IndexInfo inform,vector<Result>& res){
	string name = inform.tableName + "_" + inform.attrName + "_" + inform.name;
	BlockInfo* root = myBuffer.getBlock(dbName, name, 0, 1);
	if (root->blockNum == -2)
	{
	...
	BlockInfo* Node;
	Node = myBuffer.getBlock(dbName, name, root->blockNum, 1);
	if (Node->blockNum == -2)
	{
		cout << "block not found" << endl;
		return;
	...
	while (Node->cBlock[0] != '!'){
		...
		Node = myBuffer.getBlock(dbName, name, blocknum, 1);//调用buffermanager的函数找到下一个节点
		if (Node->blockNum == -2)
		{
			cout << "block not found" << endl;
			return;
...
void BPlusTree::searchMany(string dbName, int type, IndexInfo inform, vector<Result>& res){
	...
	BlockInfo *leaf = myBuffer.getBlock(dbName, name, blocknum, 1);
	if (leaf->blockNum == -2)
	{
		cout << "block not found" << endl;
		return;
	...
	if (type == 1){
		...
		/*不是最后一个叶子*/
			...
			if (compare >= 0){
				...
				/*offset为兄弟节点的第一个偏移量*/
				BlockInfo* next = myBuffer.getBlock(dbName, name, start, 1);
				if (next->blockNum == -2)
				{
					cout << "block not found" << endl;
					return;
	...
	/*找出所有 <inform.value的记录*/
		...
		/*不是第一个叶子*/
			...
			if (compare <= 0){
				...
				/*offset为兄弟节点的最后一个偏移量*/
				BlockInfo* prev = myBuffer.getBlock(dbName, name, end, 1);
				if (prev->blockNum == -2)
				{
					cout << "block not found" << endl;
					return;
...
void BPlusTree::getResult(string dbName, string name, int start, int end, IndexInfo inform, string Linfo, int type, vector<Result>& res){
	BlockInfo*Node = myBuffer.getBlock(dbName, name, start, 1);
	if (Node->blockNum == -2)
	{
		cout << "block not found" << endl;
		return;
	...
	for (int i = start;; i = getBlockNum(info.substr(info.size() - 3, 3))){
		Node = myBuffer.getBlock(dbName, name, i, 1);
		if (Node->blockNum == -2)
		{
			cout << "block not found" << endl;
			return;
...
void BPlusTree::insertOne(string dbName, IndexInfo  inform){
	...
	if (root->charNum == 0)
	{
		...
		BlockInfo* node = myBuffer.getBlock(dbName, name, Node, 1);
		if (node->blockNum == -2)
		{
			cout << "block not found" << endl;
			return;
		...
		if (getValueNum(info.substr(1, VBIT))<N - 1)
			...
			BlockInfo* L1 = myBuffer.getEmptyBlock(dbName, name);
			int l1 = L1->blockNum;
			/*Copy L (=node here) to T*/
			BlockInfo* T = myBuffer.getEmptyBlock(dbName, name);
			int t = T->blockNum;
			...
			//delete T
			myBuffer.deleteBlock(dbName, name, T);
		}
	}
}
void BPlusTree::insertInLeaf(string dbName, IndexInfo &inform, int Node){
	string name = inform.tableName + "_" + inform.attrName + "_" + inform.name;
	BlockInfo* leaf;
	leaf = myBuffer.getBlock(dbName, name, Node, 1);//调用buffermanager的函数找到叶子节点
	if (leaf->blockNum == -2)
	{
		cout << "block not found" << endl;
		return;
...
void BPlusTree::insertInParent(string dbName, IndexInfo inform, int Node, string K1, int N1){
	...
	if (root->blockNum == Node){
		...
		BlockInfo* R = myBuffer.getEmptyBlock(dbName, name);
		int r = R->blockNum;
		BlockInfo* node = myBuffer.getBlock(dbName, name, Node, 1);
		if (node->blockNum == -2)
		{
		...
		/*let P=parent(N)*/
		BlockInfo* N0 = myBuffer.getBlock(dbName, name, Node, 1);
		if (N0->blockNum == -2)
		{
			cout << "block not found" << endl;
			return;
		...
		BlockInfo* P = myBuffer.getBlock(dbName, name, p, 1);
		if (P->blockNum == -2)
		{
			cout << "block not found" << endl;
			return;
		...
		else
		{
			...
			P->cBlock = NULL;
			/*create node P1*/
			BlockInfo* P1 = myBuffer.getEmptyBlock(dbName, name);
			int p1 = P1->blockNum;
...
void BPlusTree::deleteEntry(string dbName, IndexInfo inform, int n, string K, int nod){
	...
	BlockInfo *Node = myBuffer.getBlock(dbName, name, n, 1);
	if (Node->blockNum == -2)
	{
		cout << "block not found" << endl;
		return;
	...
	if (Node->blockNum == 0 && originN.substr(1, 4) == "0001"){
		...
		BlockInfo* Child = myBuffer.getBlock(dbName, name, child, 1);
		if (Child->blockNum == -2)
		{
			cout << "block not found" << endl;
			return;
		...
		/*delete N*/
		myBuffer.deleteBlock(dbName, name, Node);
	}
	else{
		string info = Node->cBlock;
		tempN = Node->cBlock;
		...
		if (info[0] == '!'){
			...
			if (getValueNum(info.substr(1, 4))<leaf_least){
				...
				BlockInfo* N1 = myBuffer.getBlock(dbName, name, n1, 1);
				if (N1->blockNum == -2)
				{
					cout << "block not found" << endl;
					return;
				...
				if (father == father1){
					...
					/*redistribution*/
						...
						BlockInfo* Father = myBuffer.getBlock(dbName, name, father, 1);
						if (Father->blockNum == -2)
						{
							cout << "block not found" << endl;
							return;
						}
...
string BPlusTree::findLinfo(string dbName, IndexInfo inform, int nodenum, int type){
	string name = inform.tableName + "_" + inform.attrName + "_" + inform.name;
	string offset;
	BlockInfo* leaf = myBuffer.getBlock(dbName, name, nodenum, 1);//调用buffermanager的函数找到叶子节点
	if (leaf->blockNum == -2)
	{
		cout << "block not found" << endl;
		return "";
...
int BPlusTree::findFather(string dbName, IndexInfo inform, int num){
	...
	if (inform.type == INT)
		...
		length = LENGTH_OF_CHAR;
	int father;
	BlockInfo* node = myBuffer.getBlock(dbName, name, num, 1);
	if (node->blockNum == -2)
	{
		cout << "block not found" << endl;
		return -2;
...
int BPlusTree::findPrevLeafSibling(string dbName, IndexInfo inform, int nodenum){
	...
	if (left == nodenum)
		return -1;
	BlockInfo* Left = myBuffer.getBlock(dbName, name, left, 1);
	if (Left->blockNum == -2)
	{
		cout << "block not found" << endl;
		return -2;
	...
	while (info.substr(Left->charNum - 3, 3) != intToStr(nodenum, 3)){
		...
		Left = myBuffer.getBlock(dbName, name, left, 1);
		if (Left->blockNum == -2)
		{
			cout << "block not found" << endl;
			return -2;
...
int BPlusTree::findNextLeafSibling(string dbName, IndexInfo inform, int nodenum){
	string name = inform.tableName + "_" + inform.attrName + "_" + inform.name;
	BlockInfo* node = myBuffer.getBlock(dbName, name, nodenum, 1);
	if (node->blockNum == -2)
	{
		cout << "block not found" << endl;
		return -2;
...
int BPlusTree::findLeftestChild(string dbName, IndexInfo inform){
	...
	while (info[0] == '?')
	{
		...
		node = myBuffer.getBlock(dbName, name, getBlockNum(left), 1);
		if (node->blockNum == -2)
		{
			cout << "block not found" << endl;
			return -2;
